...
# Загрузка переменных окружения из .env (для локального теста)
load_dotenv()
...
async def record_activity(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if text.lower() == "закончить":
        ...
        await update.message.reply_text(
            "Спасибо! Все данные сохранены.", reply_markup=ReplyKeyboardRemove()
        )
        return ConversationHandler.END
...
async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Отмена диалога."""
    await update.message.reply_text(
        "Диалог отменён. Данные не сохранены.", reply_markup=ReplyKeyboardRemove()
    )
    return ConversationHandler.END
...
def main():
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            ENERGY_STATUS: [
                MessageHandler(filters.Regex("^(Даёт энергию|Забирает энергию)$"), record_energy)
            ],
        },
